...
test.only('Create shipment from carrier account', async ({ page, context }) => {
  ...
  await page.locator('#name').fill('Carrier-customer');
  await page.waitForTimeout(300); // wait until autofill completed
  ...
  // Check if deliveryAddress is what we filled in
  await expect(addressBlock.locator(':nth-child(1)')).toHaveText('Carrier-customer');
