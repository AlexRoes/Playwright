...
test('It is possible to create a shipment for a company address', async ({ page, context }) => {
  ...
  // fill order's fields
  await page.locator('#name').fill('Company');
  await page.waitForTimeout(300); // wait until autofill completed
  await page.locator('#postalCode').fill('8031 DX');
  await page.locator('#nr').fill('14');
  await page.locator('#street').fill('Zwartewaterallee');
  ...
  // Check if deliveryAddress is what we filled in
  await expect(addressBlock.locator(':nth-child(1)').first()).toHaveText('Company');
  await expect(addressBlock.locator(':nth-child(2)').first()).toHaveText('Zwartewaterallee 14');
  await expect(addressBlock.locator(':nth-child(3)')).toHaveText('8031 DX Zwolle');
