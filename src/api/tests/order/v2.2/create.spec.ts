...
test.describe('Create order flow (v2.2)', () => {
  test('should create a order as customer', async ({ request }) => {
    const newOrder = await request.post(`/api/v2.2/order`, {
      data: {
        order: baseOrder,
      },
    });
  ...
  test('should verify a order as customer by id', async ({ request }) => {
    const verifyOrder = await request.get(`/api/v2.2/order/${orderId}`);
  ...
  test('should verify a order as customer by tracktrace', async ({ request }) => {
    const verifyTracktrace = await request.get(`/api/v2.2/order/tracktrace/${trackTrace}`);
  ...
  test('should cancel a order as customer', async ({ request }) => {
    const response = await request.post(`/api/v2.2/order/cancel/${orderId}`);
